...
komterug |>
  ...
  geom_tile() +
  labs(x = "Time (s)", y = NULL, fill = "Magnitude", title= "Kom Terug") +
  theme_minimal() +
...
```{r, include=FALSE}
#making a chomagram deel 1 Astronaut
astronaut <-
  get_tidy_audio_analysis("0zluTtcWus2xlgPlz4KQrV") |>
  select(segments) |>
  unnest(segments) |>
  select(start, duration, pitches)
...
```{r, echo=FALSE}
#making a chromagram deel 2 astronaut
astronaut |>
  mutate(pitches = map(pitches, compmus_normalise, "euclidean")) |>
  compmus_gather_chroma() |> 
  ggplot(
  ...
  geom_tile() +
  labs(x = "Time (s)", y = NULL, fill = "Magnitude", title = "astronaut") +
  theme_minimal() +
  scale_fill_viridis_c() 
...
```{r, echo=FALSE}
#astronaut en kom terug
compmus_long_distance(
  astronaut |> mutate(pitches = map(pitches, compmus_normalise, "chebyshev")),
  ...
  geom_tile() +
  coord_equal() +
  labs(x = "Astronaut", y = "Kom Terug", title= "album Omnibus (2007)") +
  theme_minimal() +
